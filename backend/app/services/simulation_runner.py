...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, to truncate excessively long simulations)
        ...
        try:
            ...
            # Create main log file to avoid stdout/stderr pipe buffer blocking the process
            ...
            # Set working directory to simulation dir (databases etc. will be generated here)
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine if platform is enabled by checking if corresponding actions.jsonl file exists
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer from record, otherwise use default
    ...
    def get_all_actions(
        ...
        # If per-platform files do not exist, try old single-file format
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Note: Config files (simulation_config.json) and profile files will not be deleted
    ...
    def register_cleanup(cls):
        ...
        # If not debug mode, this env var does not exist, registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, only register in reloader child; in non-debug mode, always register
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True means alive, False means closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get detailed status of simulation environment
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform mode, interview both platforms and return integrated results
        ...
        Raises:
            ...
            TimeoutError: Response wait timeout
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation not found: {simulation_id}")
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or closed, cannot execute Interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: In dual-platform mode, interview both platforms for each Agent
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        Send close environment command for graceful exit from command waiting mode
