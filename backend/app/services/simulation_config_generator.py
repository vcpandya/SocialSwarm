...
# Multi-timezone activity pattern presets
TIMEZONE_PRESETS = {
    "asia_kolkata": {
        ...
        "peak_hours": [19, 20, 21, 22, 23],  # Indians are active later due to dinner timing
        "night_hours": [],
    ...
    "america_new_york": {
        ...
        "night_hours": [23],
        "activity_multipliers": {"dead": 0.05, "morning": 0.3, "work": 0.6, "peak": 1.5, "night": 0.4}
    },
    "america_los_angeles": {
        "label": "US West (PST, UTC-8)",
        "utc_offset": -8,
        "dead_hours": [0, 1, 2, 3, 4, 5],
        "morning_hours": [6, 7, 8],
        "work_hours": [9, 10, 11, 12, 13, 14, 15, 16, 17],
...
# Default timezone preset (backward compatible)
DEFAULT_TIMEZONE = "asia_kolkata"
...
class TimeSimulationConfig:
    ...
    # Peak hours (most active time in selected timezone)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22, 23])
    peak_activity_multiplier: float = 1.5

    # Off-peak hours (midnight/dawn, almost no activity)
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        time_config_result = self._generate_time_config(context, num_entities, timezone=timezone)
    ...
    def _generate_time_config(self, context: str, num_entities: int, timezone: str = None) -> Dict[str, Any]:
        """Generate time configuration"""
        # Resolve timezone preset
        tz_key = timezone or DEFAULT_TIMEZONE
        tz_preset = TIMEZONE_PRESETS.get(tz_key, TIMEZONE_PRESETS[DEFAULT_TIMEZONE])
        tz_label = tz_preset["label"]
...
- Activity patterns follow {tz_label} local time
- Midnight 0-5: almost no activity (activity multiplier 0.05)
- Morning 6-8: gradually active (activity multiplier 0.4)
- Work hours 9-18: moderately active (activity multiplier 0.7)
- Evening peak: most active period (activity multiplier 1.5)
- Late night: activity declining (activity multiplier 0.5)
- General pattern: low activity at dawn, gradually increasing in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only; you should adjust time segments based on the nature of the event and participant group characteristics
...
{{
    ...
    "peak_hours": {json.dumps(tz_preset["peak_hours"])},
    "off_peak_hours": {json.dumps(tz_preset["dead_hours"])},
    "morning_hours": {json.dumps(tz_preset["morning_hours"])},
    "work_hours": {json.dumps(tz_preset["work_hours"])},
    "reasoning": "Time configuration explanation for this event"
}}

Field descriptions:
- timezone (string): Timezone preset key, use "{tz_key}" for {tz_label}
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = f"You are a social media simulation expert. Return pure JSON format. Time configuration should follow {tz_label} activity patterns."

        try:
            return self._call_llm_with_retry(prompt, system_prompt)
        except Exception as e:
            logger.warning(f"Time config LLM generation failed: {e}, using default config")
            return self._get_default_time_config(num_entities, timezone=timezone)
    
    def _get_default_time_config(self, num_entities: int, timezone: str = None) -> Dict[str, Any]:
        """Get default time configuration for the selected timezone"""
        tz_key = timezone or DEFAULT_TIMEZONE
        tz_preset = TIMEZONE_PRESETS.get(tz_key, TIMEZONE_PRESETS[DEFAULT_TIMEZONE])
        return {
            "timezone": tz_key,
            ...
            "peak_hours": tz_preset["peak_hours"],
            "off_peak_hours": tz_preset["dead_hours"],
            "morning_hours": tz_preset["morning_hours"],
            "work_hours": tz_preset["work_hours"],
            "reasoning": f"Using default {tz_preset['label']} activity pattern (1 hour per round)"
        }
...
For example: official statements should be published by Official/University types, news by MediaOutlet, student perspectives by Student.
...
- **Time follows Beijing timezone (UTC+8) patterns**: almost no activity 0-5, most active 19-22
...
- **Media** (MediaOutlet): moderate activity (0.4-0.6), all-day active (8-23), fast response (5-30 min), high influence (2.0-2.5)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, following Beijing time patterns>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should follow Beijing time (UTC+8) activity patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent config by rule-based patterns"""
