        ...
        <div class="card-content">
          ...
          <div class="config-item" style="margin-top: 12px;">
            ...
            <select v-model="selectedTimezone" class="timezone-select">
              <option value="asia_kolkata">{{ $t('step2.tz_asia_kolkata') }}</option>
              <option value="asia_shanghai">{{ $t('step2.tz_asia_shanghai') }}</option>
              <option value="america_new_york">{{ $t('step2.tz_america_new_york') }}</option>
              <option value="america_los_angeles">{{ $t('step2.tz_america_los_angeles') }}</option>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.genAgentPersonas') }}</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">{{ $t('step2.expectedAgentCount') }}</span>
            </div>
            <div class="stat-card">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.timezone') }}</span>
                  <span class="config-item-value">{{ $t('step2.tz_' + selectedTimezone) }}</span>
                </div>
                <div class="config-item">
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">{{ $t('step2.platform1Plaza') }}</span>
                  </div>
                ...
                <div v-if="simulationConfig.reddit_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">{{ $t('step2.platform2Topic') }}</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">{{ $t('step2.recencyWeight') }}</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.prepComplete') }}</span>
          </div>
          <div class="step-status">
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">{{ $t('step2.simRoundsConfig') }}</span>
                <span class="section-desc">{{ $t('step2.simRoundsDesc') }} <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> {{ $t('step2.hours') }}{{ $t('step2.eachRoundRepresents') }} <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> {{ $t('step2.minuteTimeFlow') }}</span>
              </div>
              <label class="switch-control">
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  <div class="slider-main-value">
                    ...
                    <span class="val-unit">{{ $t('step2.roundsUnitLabel') }}</span>
                  </div>
                  <div class="slider-meta-info">
                    <span>{{ $t('step2.estDuration100Agents', { minutes: Math.round(customMaxRounds * 0.6) }) }}</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.estDuration100Agents', { minutes: Math.round(autoGeneratedRounds * 0.6) }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">{{ $t('step2.eventVisibleAge') }}</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} {{ $t('step2.ageUnit') }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ $t('step2.eventVisibleGender') }}</span>
            ...
            <div class="info-item">
              <span class="info-label">{{ $t('step2.eventVisibleMBTI') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.eventPanorama') }}</span>
                <span class="dim-desc">{{ $t('step2.eventPanoramaDesc') }}</span>
              </div>
              <div class="dimension-card">
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(t('step2.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // User keeps auto-generated rounds, don't pass max_rounds parameter
    addLog(t('step2.startSimAutoRounds', { rounds: autoGeneratedRounds.value }))
  }
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Set expected Agent total count immediately (from prepare API response)
      ...
      addLog(t('step2.startPollingProgress'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(t('step2.startGenAgentPersonas'))
        }
        addLog(`→ ${t('step2.agentPersonaProgress', { current: currentCount, total: total, name: profileName, profession: latestProfile?.profession || t('step2.unknownProfession') })}`)
        
        // If all generation is complete
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ ${t('step2.allAgentPersonasDone', { count: currentCount })}`)
        }
      }
    }
  } catch (err) {
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog(t('step2.genAgentPersonaConfig'))
        } else if (data.generation_stage === 'generating_config') {
          addLog(t('step2.callingLLMGenConfig'))
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        addLog(`✓ ${t('step2.simConfigGenComplete')}`)

        // Show detailed config summary
        if (data.summary) {
          addLog(`  ├─ ${t('step2.agentCountSummary', { count: data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.simDurationSummary', { hours: data.summary.simulation_hours })}`)
          addLog(`  ├─ ${t('step2.initialPostsSummary', { count: data.summary.initial_posts_count })}`)
          addLog(`  ├─ ${t('step2.hotTopicsSummary', { count: data.summary.hot_topics_count })}`)
          addLog(`  └─ ${t('step2.platformConfigSummary', { twitter: data.summary.has_twitter_config ? '✓' : '✗', reddit: data.summary.has_reddit_config ? '✓' : '✗' })}`)
        }

        // Show time config details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(t('step2.timeConfigSummary', { minutes: tc.minutes_per_round, rounds: Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round) }))
        }
        
        // Show event config
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(t('step2.narrativeDirectionLog', { text: narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative }))
        }
...
const loadPreparedData = async () => {
  ...
  addLog(t('step2.loadedAgentPersonas', { count: profiles.value.length }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog(`✓ ${t('step2.simConfigLoaded')}`)

        // Show detailed config summary
        if (res.data.summary) {
          addLog(`  ├─ ${t('step2.agentCountSummary', { count: res.data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.simDurationSummary', { hours: res.data.summary.simulation_hours })}`)
          addLog(`  └─ ${t('step2.initialPostsSummary', { count: res.data.summary.initial_posts_count })}`)
        }
        ...
        addLog(t('step2.configGenPolling'))
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(t('step2.loadConfigFailed', { error: err.message }))
    emit('update-status', 'error')
  }
}
