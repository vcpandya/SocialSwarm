...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`${t('step3.stopFailed')}: ${res.error || t('step3.unknownError')}`)
    }
  } catch (err) {
    addLog(`${t('step3.stopException')}: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Track last round per platform for detecting changes and outputting logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Extra check: if backend hasn't updated runner_status yet, but platforms have reported completion
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ ${t('step3.reportStartFailed')}: ${res.error || t('step3.unknownError')}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ ${t('step3.reportStartException')}: ${err.message}`)
    isGeneratingReport.value = false
  }
}
