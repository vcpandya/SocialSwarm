  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="$t('process.refreshGraph')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('process.graphRAGUpdating') : $t('process.realtimeUpdating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('process.hintProcessing') }}</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" :title="$t('process.closeHint')">
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>{{ $t('process.graphLoading') }}</p>
      </div>
      
      <!-- Waiting/Empty State -->
      <div v-else class="graph-state">
        <div class="empty-icon">❖</div>
        <p class="empty-text">{{ $t('process.waitingOntology') }}</p>
      </div>
    </div>
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number and index of edges between each pair of nodes
  const edgePairCount = {}
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to connection direction, calculated by distance ratio, ensuring visible curves
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether drag actually started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First true drag detected, restart simulation
          d._isDragging = true
