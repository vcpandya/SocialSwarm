            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ $t('step4.generatingSection', { section: section.title }) }}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation done, but entire section may not be complete yet) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-questions - full extraction, no quantity limit
    ...
    // Extract key facts - full extraction, no quantity limit
    ...
    // Extract relation chains - full extraction, no quantity limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction, no quantity limit
    const activeSection = text.match(/### Active Facts\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extract historical/expired facts - full extraction, no quantity limit
    ...
    // Extract related entities - full extraction, no quantity limit
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (excluding concluding summary paragraphs)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - full extraction, no quantity limit
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" (Chinese format, backend new format)
      // 2. "1. " or "\n1. " (number+dot, old format compatible)
      let matches = []
      let match

      // First try "QuestionX:" (Chinese) format
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - question and answer style
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  h('div', {
                    ...
                    innerHTML: isPlaceholder
                      ? answerText
                      : sanitizeHtml(formatAnswer(answerText, isExpanded)
...
// Currently active step (used for top display)
...
const extractFinalContent = (response) => {
  ...
  // If starts with ## or # or >, it's likely direct markdown content
