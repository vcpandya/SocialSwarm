      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('process.graphLoading') }}</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.generatedEntityTypes', { count: projectData.ontology.entity_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.generatedRelationTypes', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waitingOntologyGen') }}</div>
              </div>
            </div>
          </div>
...
const goToNextStep = () => {
  ...
  alert(t('process.envSetupDevInProgress'))
}
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = t('process.projectInitFailed', { error: err.message || t('process.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = t('process.loadProjectFailed', { error: err.message || t('process.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = t('process.startGraphBuildFailed', { error: err.message || t('process.unknownError') })
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailed', { error: task.error || t('process.unknownError') })
        buildProgress.value = null
      }
    }
  } catch (err) {
